...
#pragma unroll
  for (int i = 0; i < 4; ++i) {
    ...
    BLdsOffset[i] =
        (tmp + 0x2000 / 16 + (slot & 1) * 0x1000 / 16) * 16;
  }
...
#pragma unroll
  for (int stage_i = 0; stage_i < Stage; ++stage_i) {
    WsmUint4 a_frag0 =
        *reinterpret_cast<WsmUint4 *>(WSM_lds + 0x4000 * stage_i + ALdsOffset[0]);
    WsmUint4 a_frag1 =
        *reinterpret_cast<WsmUint4 *>(WSM_lds + 0x4000 * stage_i + ALdsOffset[1]);
    WsmUint4 a_frag2 =
        *reinterpret_cast<WsmUint4 *>(WSM_lds + 0x4000 * stage_i + ALdsOffset[2]);
    WsmUint4 a_frag3 =
        *reinterpret_cast<WsmUint4 *>(WSM_lds + 0x4000 * stage_i + ALdsOffset[3]);
    WsmUint4 b_frag0 =
        *reinterpret_cast<WsmUint4 *>(WSM_lds + 0x4000 * stage_i + BLdsOffset[0]);
    WsmUint4 b_frag1 =
        *reinterpret_cast<WsmUint4 *>(WSM_lds + 0x4000 * stage_i + BLdsOffset[1]);
    WsmUint4 b_frag2 =
        *reinterpret_cast<WsmUint4 *>(WSM_lds + 0x4000 * stage_i + BLdsOffset[2]);
    WsmUint4 b_frag3 =
        *reinterpret_cast<WsmUint4 *>(WSM_lds + 0x4000 * stage_i + BLdsOffset[3]);
...
#pragma unroll
      for (int col = 0; col < 4; ++col) {
        C_f32[row][col] = __builtin_mxc_mma_16x16x16f16(
            mma_a[row], mma_b[col], C_f32[row][col]);
      }
    }
  }
