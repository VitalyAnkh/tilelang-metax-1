...
struct Barrier {
  ...
  TL_DEVICE void init(uint32_t arrive_count) { tl::mbarrier_init(value, arrive_count); }
  TL_DEVICE void arrive() { tl::mbarrier_arrive(value); }
  TL_DEVICE void arrive(int cta_id, uint32_t pred) { tl::mbarrier_arrive(value, cta_id, pred); }
